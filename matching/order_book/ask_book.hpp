...
public:
    ...
    AskBook(const AskBook&) = delete;
    AskBook& operator=(const AskBook&) = delete;
    ...
    // Creates and links a new price level in sorted ascending order (lowest ask first)
    PriceLevelNode* create_level(const Price& price, ObjectPool<PriceLevelNode, 1024>& level_pool) noexcept {
        ...
        if (!best_level_) {
            ...
            // New best ask (lower price than current best ask)
            new_node->next = best_level_;
            best_level_->prev = new_node;
            best_level_ = new_node;
        } else {
            // Traverse up from best_level_ to find insertion slot
    ...
    // Asserts all sell-side invariants
    void verify_invariants() const noexcept {
        ...
        while (curr) {
            assert(curr->price >= prev_price); // Must ascend
...
private:
    ...
    PriceLevelIndex<std::less<Price>> index_;
};
